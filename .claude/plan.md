...
## Architecture
```
Claude Code (200K context) → stdio → MCP Server (Python/FastMCP) → HTTP → apfel --serve (localhost:11434) → Apple Neural Engine (3B LLM)
```

Transport rule: MCP server uses stdio for Claude Code, HTTP for apfel. Never mix them.
...
3. **summarize_history(turns)** — compress old conversation turns, chunked for 4K window
4. **generate_handoff(token_budget)** — create session digest from state files for new sessions
...
Four system prompt .txt files exist: `compress.txt`, `classify.txt`, `summarize.txt`, `extract.txt`.
**Note:** `extract.txt` is currently unused — there is no `ApfelClient.extract()` method. It was generated by Copilot speculatively. Either implement the method or delete the file.

## Current Status
- v0.2.1 — bench.py warmup/TTFT support + gemma4 context window fix
- Backend: Gemma 4 E4B via Ollama (128K ctx, modelfile updated), configured via config.json
- Tests: 79/79 passing
- Hooks: SessionStart (bounded state injection), UserPromptSubmit (auto-compact prose), PostToolUse/Edit (log edits), Stop (log turn marker)
...
## Known Issues (open)
- `extract.txt` prompt file has no corresponding `ApfelClient.extract()` method — unused dead code. Delete or implement.
...
- Implement `ApfelClient.extract()` or delete `extract.txt`
- Per-file codebase summaries that update on save
